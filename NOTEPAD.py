...
class Notepad:
    ...
    def __showAbout(self):
        showinfo("Notepad","Prajwal")
    def __openFile(self):
        self.__file = askopenfilemenu(defaultextension=".txt", filetypes=[("All Files","*.*"),("Text Document","*.txt")])
        if self.__file == "":
    ...
    def __saveFile(self):
        if self.__file==None:
            ...
            file=open(self.__file,"w")
            file.write(self.__thisTextArea.gert(1.0,END))
            file.close()
    def __cut(self):
